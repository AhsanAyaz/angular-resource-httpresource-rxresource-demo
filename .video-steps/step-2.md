# Step 2: httpResource() with a URL function

Slides: Part 2 (httpResource(): the HttpClient-powered shortcut, URL function reading a signal, parse()).
Goal: fill in the second example. A URL function reads a signal, so flipping the toggle
re-fetches automatically through HttpClient. A parse() step maps the multi-city payload.

## FILE: src/app/http-resource-example/http-resource-example.component.ts
```ts
...
- `httpResource(urlFn, options)` is the HttpClient-powered shortcut: no manual loader.
- The URL function reads `isMultiCityMode()`, so flipping the toggle re-fetches automatically.
- `parse` maps the raw response and can `throw` to surface a validation error in `error()`.
- Same signal surface as before: `value()`, `isLoading()`, `error()`.

Try it: flip the "Multi-city mode (Milan)" toggle and watch it re-fetch.
```
